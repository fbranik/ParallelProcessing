...
int main(int argc, char **argv)
{
	...
	gettimeofday(&t1,0);
	//#pragma omp parallel shared(A, N, B)
            ...
            int myN, int bsize)
{
	...
	if(myN<=bsize)
		...
		#pragma omp parallel shared(A, B, C, myN, bsize) private(i, j, k) firstprivate(arow, acol, brow, bcol, ccol, crow)
		{
			#pragma omp single
			{
				#pragma omp task // task creation is also defined as a task because of recursive calls 
				{
