    ...
    if (iBlock < numClusterBlocks) {
        if (iCluster < numClusters) {
            ...
            for (iCoord = 0; iCoord < numCoords; iCoord++) {
                atomicAdd(&deviceNewClusters[numClusters * iCoord + iCluster], deviceLocalNewClusters[iBlock * numClusters * numCoords + iCoord * numClusters + iCluster]);
            }
        }
    }
}
    ...
    if (iCluster < numClusters) {
        if (iCoord < numCoords) {
            if (shmemNewClusterSize[iCluster] > 0) {
                deviceClusters[iCoord * numClusters + iCluster] = shmemNewClusters[iCoord * numClusters + iCluster] / shmemNewClusterSize[iCluster];
            }
        }
    }
}
...
void kmeans_gpu(float *objects,      /* in: [numObjs][numCoords] */
    ...
    int *deviceMembership, *deviceLocalNewClusterSize, *deviceNewClusterSize;

    printf("\n|-----------All GPU Kmeans (ParallelBlockReduction)------------|\n\n");
    ...
    dim3 numThreadsPerBlockReduction(64,16,1);
    dim3 numBlocksReduction(numClusterBlocks/64, numClusters/16, 1);
    ...
    checkCuda(cudaMalloc(&deviceNewClusterSize, numClusterBlocks * numClusters * sizeof(int)));
    ...
    do {
        ...
        reduce_newClusterData<<<numBlocksReduction, numThreadsPerBlockReduction>>>
                (numCoords, numClusters, numClusterBlocks,
                 deviceLocalNewClusterSize, deviceLocalNewClusters, deviceNewClusters, deviceNewClusterSize);

        cudaDeviceSynchronize();
        checkLastCudaError();

        update_centroids<<<numBlocksUpdateCentroids, numThreadsPerBlockUpdateCentroids, clusterBlockSharedDataSizeUpdateCentroids>>>
                (numCoords, numClusters,
                 deviceClusters, deviceNewClusters, deviceNewClusterSize);
