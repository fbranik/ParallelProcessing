...
__device__ void newClusterDataReduction(int numCoords, int numClusters,
										int * deviceNewClusterSize, float * deviceNewClusters,
										int * deviceLocalNewClusterSize, float * deviceLocalNewClusters){
	int tid = get_tid();
	int iCoord, iCluster, iBlock;
	if (tid < numClusters){	
		for(iBlock=0; iBlock<gridDim.x; iBlock++){
		//	atomicAdd(&
		deviceNewClusterSize[tid] += deviceLocalNewClusterSize[iBlock*numClusters+tid];
			for (iCoord=0; iCoord<numCoords; iCoord++){
				//atomicAdd(&
				deviceNewClusters[numClusters*iCoord+tid] += deviceLocalNewClusters[iBlock*numClusters*numCoords+iCoord*numClusters+tid];
			}
		}
	}	
}
						  ...
						  float *deviceLocalNewClusters)
{
	...
	/*
    	Shared Memory Layout (Each block has this layout and reduction is performed later):
    ...
    */	
	...
	if (threadIdx.x == 0){
		// The first thread of each block copies the shared memory copies 
		// of its block to global memory locations so that reduction can be performed
		memcpy(&deviceLocalNewClusterSize[blockIdx.x*numClusters], shmemNewClusterSize, numClusters*sizeof(int));
		memcpy(&deviceLocalNewClusters[blockIdx.x*numCoords*numClusters], shmemNewClusters, numClusters*numCoords*sizeof(float));		
		//__syncthreads();
		//reset shared memory to 0 to reuse it as the storage for reduction
		memset(shmemNewClusterSize, 0, numClusters*sizeof(int));
    	memset(shmemClusters, 0, numClusters*numCoords*sizeof(float));	
	}
	__syncthreads();
	newClusterDataReduction(numCoords, 				   numClusters,
							deviceNewClusterSize, 	   deviceNewClusters,
							deviceLocalNewClusterSize, deviceLocalNewClusters);
	__syncthreads();
	if (tid<numClusters) {
    	for (i=0; i<numCoords; i++) {
        	if (deviceNewClusterSize[tid] > 0){
            	deviceClusters[i*numClusters+tid] = deviceNewClusters[i*numClusters+tid] / deviceNewClusterSize[tid];	
			}
		}
	}
}
						...
						int blockSize)  
{
	...
	float *deviceObjects;
    float *deviceClusters, *deviceNewClusters,    *deviceLocalNewClusters;
    int *deviceMembership, *deviceNewClusterSize, *deviceLocalNewClusterSize;
    ...
    do {
        ...
        checkCuda(cudaMemset(deviceNewClusters, 0, numClusters*numCoords*sizeof(float)));          
